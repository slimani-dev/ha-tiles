...
class ManagedSlider {
    constructor(onCommit) {
        ...
        this.actor.connectObject(
            ...
            'drag-end', () => this._commit(),
            'destroy', () => this._onDestroy(),
            this);
    }

    // The actor is destroyed together with its menu item; connectObject() drops the
    // signal connections itself, so only the pending commit and the reference remain
    _onDestroy() {
        this._clearTimer();
        this.actor = null;
    }
