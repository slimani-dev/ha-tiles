...
class EntityItem extends PopupMenu.PopupBaseMenuItem {
    ...
    _buildControls(body) {
        ...
        case 'light':
            if (c.brightness) {
                ...
                body.add_child(this._slider.actor);
            }
            if (c.color || c.colorTemp) {
                ...
                this.add_child(iconButton(this._ctx, 'palette', 'Color', () => {
        ...
        case 'cover':
            ...
            if (c.open)
                this.add_child(iconButton(this._ctx, 'arrow-up', 'Open', () => client.callService('cover', 'open_cover', id)));
            if (c.stop)
                this.add_child(iconButton(this._ctx, 'stop', 'Stop', () => client.callService('cover', 'stop_cover', id)));
            if (c.close)
                this.add_child(iconButton(this._ctx, 'arrow-down', 'Close', () => client.callService('cover', 'close_cover', id)));
            break;
        case 'number':
        case 'input_number': {
            this._slider = new ManagedSlider(v => {
        ...
        case 'media_player':
            ...
            if (c.playPause) {
                ...
                this.add_child(this._playButton);
            }
            break;
        case 'scene':
        case 'script':
    ...
    activate() {
        ...
        default:
            if (E.isToggleable(id)) {
                this._switch.state = !this._switch.state; // optimistic
                client.callService('homeassistant', this._switch.state ? 'turn_on' : 'turn_off', id);
            }
        }
    }
    ...
    sync() {
        ...
        if (this._switch) {
            this._switch.state = on;
            this._switch.visible = !unavailable;
        }
        if (this._slider) {
            let value = 0;
            switch (this._domain) {
...
class SectionHeaderItem extends PopupMenu.PopupBaseMenuItem {
    constructor(ctx, section) {
        ...
        this._icon = new St.Icon({gicon: ctx.icons.lookup(section.icon), style_class: 'popup-menu-icon haqs-section-icon'});
        ...
        if (groupControls && (colorIds.length >= 2 || tempIds.length >= 2)) {
            ...
            this.add_child(iconButton(ctx, 'palette', 'Color for the whole section', () => {
                this.colorPanel.visible = !this.colorPanel.visible;
            }));
        }
    ...
    activate() {
        ...
        this._ctx.client.callService('homeassistant', this._switch.state ? 'turn_on' : 'turn_off', this._toggleIds);
    }
