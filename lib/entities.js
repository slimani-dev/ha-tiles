...
export const DEFAULT_ICONS = [
    ...
    'arrow-up', 'stop', 'arrow-down', 'play-pause', 'open-in-new',
];
//#endregion Icons
...
function customGroupMembers(client, group) {
    ...
    const devices = new Set(group.devices ?? []);
    if (devices.size) {
        for (const [id, e] of client.entities) {
            if (devices.has(e.deviceId) && !e.category)
                members.add(id);
        }
    }
    return members;
...
export function clusterByDevice(client, entities) {
    const clusters = [];
